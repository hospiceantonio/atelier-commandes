...
const VueClients = (() => {
          ...
          '<button type="button" class="ligne" data-nav="#/client/' + c.id + '">' +
            ...
            '<span class="ligne-corps">' +
              ...
              '<span class="ligne-sous">' +
                (c.tel ? "<span>" + UI.icone("tel", "ic-sm") + " " + e(Utils.fmtTel(c.tel)) + "</span>" : "<span>Sans téléphone</span>") +
              "</span>" +
            "</span>" +
  ...
  async function fiche(vue, id) {
    ...
    UI.entete({
      ...
      sous: client.tel ? Utils.fmtTel(client.tel) : "Sans téléphone",
      retour: true,
    ...
    if (client.tel) {
      html +=
        '<div class="btn-rangee">' +
          '<a class="btn btn-wa" href="' + e(Utils.lienWhatsApp(client.tel, "", r.indicatif)) + '" target="_blank" rel="noopener">' +
            UI.icone("whatsapp", "ic-sm") + "WhatsApp</a>" +
          '<a class="btn btn-clair" href="' + e(Utils.lienTel(client.tel, r.indicatif)) + '">' +
    ...
    vue.innerHTML =
      '<form id="form-client" novalidate>' +
        '<div class="carte">' +
          ...
          "</div>" +
          '<div class="champ"><label for="cli-tel">Téléphone (WhatsApp)</label>' +
            '<input id="cli-tel" type="tel" inputmode="tel" autocomplete="off" placeholder="97 00 00 00" value="' + e(client ? client.tel : "") + '">' +
            '<div class="aide">Sert aux messages WhatsApp et aux appels en un geste.</div></div>' +
